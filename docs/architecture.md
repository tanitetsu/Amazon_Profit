...
- 明細の販売額はメールの「価格」+「税金」を **税込価格** へ合算（ライブテンプレが税金列なしのため。分割は移行まで停止）。Pt 欠落時は税込価格×1%
...
公開ステータスが **Testing** だと運営／ユーザーの refresh token は **7 日で失効**する。In production なら期限の固定はなく、5 分ポーリングの keep-alive が 6 ヶ月未使用の無効化を防ぐ。`oauth_operator.py` の再同意はトークン失効時だけ（同一クライアントの refresh 上限対策）。
